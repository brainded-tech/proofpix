...
- Safe for use in legal proceedings

---
Generated by ProofPix Enterprise
Report ID: ${Date.now()}`;
      ...
      a.href = url;
      a.download = `proofpix-report-${image.name}-${Date.now()}.txt`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      alert('Analysis report generated and downloaded successfully!');
    } catch (error) {
        ...
        custodyChain: [
          {
            ...
            user: 'Current User',
            location: 'ProofPix Platform',
            hash: image.metadata.hash
          },
          {
            action: 'ANALYZED',
            timestamp: new Date().toISOString(),
            user: 'ProofPix System',
            location: 'Secure Processing Environment',
            hash: image.metadata.hash
          }
        ]
      };
...
Image: ${image.name}
File Hash: ${image.metadata.hash}
Generated: ${new Date().toLocaleString()}

CUSTODY EVENTS:
${custodyData.custodyChain.map((event, index) => `
${index + 1}. ${event.action}
   Timestamp: ${new Date(event.timestamp).toLocaleString()}
   User: ${event.user}
   Location: ${event.location}
   Hash Verification: ${event.hash}
`).join('')}
...
ProofPix Enterprise Platform`;

      // Create and download the custody record
      const blob = new Blob([custodyContent], { type: 'text/plain' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `chain-of-custody-${image.name}-${Date.now()}.txt`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      alert('Chain of custody record generated and downloaded!');
    } catch (error) {
      console.error('Error generating chain of custody:', error);
      alert('Failed to generate chain of custody record. Please try again.');
    }
  };
  ...
  const handleExportMetadata = (image: any) => {
    try {
      const metadataExport = {
        fileName: image.name,
        fileSize: image.size,
        uploadDate: image.metadata.timestamp,
        hash: image.metadata.hash,
        metadata: {
          basic: {
          ...
          location: {
            ...
            address: image.metadata.location,
            altitude: '52m'
          },
          technical: {
            colorSpace: 'sRGB',
        ...
        exportedBy: 'ProofPix Enterprise'
      };

      // Create and download JSON export
      const blob = new Blob([JSON.stringify(metadataExport, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `metadata-${image.name}-${Date.now()}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      alert('Metadata exported successfully as JSON file!');
    } catch (error) {
  ...
  const handleCompareImages = (image: any) => {
    alert(`Image comparison feature coming soon! 
    
This will allow you to:
- Compare metadata between multiple images
- Detect potential duplicates or variations
- Analyze editing history
