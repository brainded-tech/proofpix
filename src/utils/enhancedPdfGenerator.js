...
class EnhancedPdfGenerator {
    ...
    try {
      ...
      switch (template) {
        ...
        case 'detailed':
          ...
          break;
        case 'comparison':
          currentY = await this.comparisonTemplate(pdf, data, currentY, options);
          break;
        default:
          currentY = await this.professionalTemplate(pdf, data, currentY, options);
      }
  ...
  async comparisonTemplate(pdf, data, startY, options = {}) {
    let y = startY;
    y = await this.addComparisonHeader(pdf, data, y, options);
    y = await this.addSideBySideImages(pdf, data, y, options);
    y = await this.addComparisonSummary(pdf, data, y);
    y = await this.addDifferencesTable(pdf, data, y);
    y = await this.addSimilaritiesTable(pdf, data, y);
    y = await this.addComparisonConclusion(pdf, data, y);
    return y;
  }
  ...
  async addComparisonHeader(pdf, data, y, options = {}) {
    const { pageWidth, margin } = this.getPageDimensions(pdf);

    // Main title
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(20);
    pdf.setTextColor(147, 51, 234); // Purple color for comparison
    const title = 'Image Comparison Analysis Report';
    const titleWidth = pdf.getTextWidth(title);
    pdf.text(title, (pageWidth - titleWidth) / 2, y);
    y += 25;
    ...
    const reportDate = new Date().toLocaleDateString();
    const leftImageName = data.comparisonData?.leftImage?.metadata?.fileName || 'Image A';
    const rightImageName = data.comparisonData?.rightImage?.metadata?.fileName || 'Image B';
    
    pdf.text(`Report Date: ${reportDate}`, margin + 5, y + 10);
    pdf.text(`Image A: ${leftImageName}`, margin + 5, y + 20);
    pdf.text(`Image B: ${rightImageName}`, margin + 5, y + 30);
    pdf.text(`Comparison ID: CMP-${Date.now()}`, margin + 100, y + 10);
    pdf.text(`Generated by: ProofPix Comparison Tool`, margin + 100, y + 20);

    return y + 45;
  }

  async addSideBySideImages(pdf, data, y, options = {}) {
    const { pageWidth, margin } = this.getPageDimensions(pdf);

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(14);
    pdf.setTextColor(0, 0, 0);
    pdf.text('Visual Comparison', margin, y);
    y += 15;
    ...
    pdf.rect(margin, y, imageWidth, imageHeight, 'F');
    pdf.setDrawColor(203, 213, 225);
    pdf.rect(margin, y, imageWidth, imageHeight, 'S');

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(12);
    pdf.setTextColor(71, 85, 105);
    pdf.text('Image A', margin + 5, y + 10);

    // Add image details
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(9);
    const leftImage = data.comparisonData?.leftImage;
    if (leftImage) {
      pdf.text(`File: ${leftImage.metadata?.fileName || 'Unknown'}`, margin + 5, y + 25);
      pdf.text(`Size: ${leftImage.metadata?.fileSize || 'Unknown'}`, margin + 5, y + 35);
      pdf.text(`Camera: ${leftImage.metadata?.make || 'Unknown'} ${leftImage.metadata?.model || ''}`, margin + 5, y + 45);
      pdf.text(`Date: ${leftImage.metadata?.dateTime || 'Unknown'}`, margin + 5, y + 55);
    }

    // Right image placeholder
    const rightX = margin + imageWidth + margin;
    pdf.setFillColor(248, 250, 252);
    pdf.rect(rightX, y, imageWidth, imageHeight, 'F');
    pdf.setDrawColor(203, 213, 225);
    pdf.rect(rightX, y, imageWidth, imageHeight, 'S');

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(12);
    pdf.setTextColor(71, 85, 105);
    pdf.text('Image B', rightX + 5, y + 10);

    // Add image details
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(9);
    const rightImage = data.comparisonData?.rightImage;
    if (rightImage) {
      pdf.text(`File: ${rightImage.metadata?.fileName || 'Unknown'}`, rightX + 5, y + 25);
      pdf.text(`Size: ${rightImage.metadata?.fileSize || 'Unknown'}`, rightX + 5, y + 35);
      pdf.text(`Camera: ${rightImage.metadata?.make || 'Unknown'} ${rightImage.metadata?.model || ''}`, rightX + 5, y + 45);
      pdf.text(`Date: ${rightImage.metadata?.dateTime || 'Unknown'}`, rightX + 5, y + 55);
    }

    return y + imageHeight + 20;
  }

  async addComparisonSummary(pdf, data, y) {
    const { pageWidth, margin } = this.getPageDimensions(pdf);

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(14);
    pdf.setTextColor(0, 0, 0);
    pdf.text('Comparison Summary', margin, y);
    y += 15;
    ...
    const similarities = comparisonData?.similarities?.length || 0;
    const differences = comparisonData?.differences?.length || 0;
    const total = similarities + differences;
    const similarityScore = total > 0 ? Math.round((similarities / total) * 100) : 0;

    pdf.text(`Similarities Found: ${similarities}`, margin + 10, y + 15);
    pdf.text(`Differences Found: ${differences}`, margin + 10, y + 25);
    pdf.text(`Overall Similarity Score: ${similarityScore}%`, margin + 10, y + 35);

    // Add distance if GPS data available
    if (comparisonData?.detailedComparison?.location?.distance) {
      const distance = comparisonData.detailedComparison.location.distance;
      pdf.text(`GPS Distance: ${distance.toFixed(2)} km apart`, margin + 120, y + 15);
    }

    return y + 50;
  }

  async addDifferencesTable(pdf, data, y) {
    const { pageWidth, margin } = this.getPageDimensions(pdf);

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(14);
    pdf.setTextColor(220, 38, 38); // Red color for differences
    pdf.text('Key Differences', margin, y);
    y += 15;

    const differences = data.comparisonData?.differences || [];
    
    if (differences.length === 0) {
      pdf.setFont('helvetica', 'italic');
      pdf.setFontSize(10);
      pdf.setTextColor(107, 114, 128);
      pdf.text('No significant differences found between the images.', margin, y);
      return y + 15;
    }

    // Differences box
    const boxHeight = Math.min(differences.length * 8 + 10, 60);
    pdf.setFillColor(254, 242, 242);
    ...
    differences.slice(0, 7).forEach((difference, index) => {
      const text = `• ${difference}`;
      const lines = pdf.splitTextToSize(text, pageWidth - (margin * 3));
      pdf.text(lines[0], margin + 5, y + 10 + (index * 8));
    });

    return y + boxHeight + 15;
  }

  async addSimilaritiesTable(pdf, data, y) {
    const { pageWidth, margin } = this.getPageDimensions(pdf);

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(14);
    pdf.setTextColor(34, 197, 94); // Green color for similarities
    pdf.text('Key Similarities', margin, y);
    y += 15;

    const similarities = data.comparisonData?.similarities || [];
    
    if (similarities.length === 0) {
      pdf.setFont('helvetica', 'italic');
      pdf.setFontSize(10);
      pdf.setTextColor(107, 114, 128);
      pdf.text('No significant similarities found between the images.', margin, y);
      return y + 15;
    }

    // Similarities box
    const boxHeight = Math.min(similarities.length * 8 + 10, 60);
    pdf.setFillColor(240, 253, 244);
    pdf.rect(margin, y, pageWidth - (margin * 2), boxHeight, 'F');
    pdf.setDrawColor(34, 197, 94);
    pdf.rect(margin, y, pageWidth - (margin * 2), boxHeight, 'S');
    ...
    similarities.slice(0, 7).forEach((similarity, index) => {
  ...
  async addComparisonConclusion(pdf, data, y) {
    const { pageWidth, margin } = this.getPageDimensions(pdf);

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(14);
    pdf.setTextColor(0, 0, 0);
    pdf.text('Analysis Conclusion', margin, y);
    y += 15;

    pdf.setFillColor(249, 250, 251);
    pdf.rect(margin, y, pageWidth - (margin * 2), 50, 'F');
    pdf.setDrawColor(229, 231, 235);
    pdf.rect(margin, y, pageWidth - (margin * 2), 50, 'S');
    ...
    if (similarityScore >= 80) {
      conclusion = 'The images show high similarity and are likely related or taken under similar conditions.';
    } else if (similarityScore >= 50) {
      conclusion = 'The images show moderate similarity with some notable differences.';
    } else {
      conclusion = 'The images show significant differences and may be unrelated or taken under different conditions.';
    }

    const conclusionLines = pdf.splitTextToSize(conclusion, pageWidth - (margin * 3));
    conclusionLines.forEach((line, index) => {
      pdf.text(line, margin + 5, y + 15 + (index * 6));
    });

    pdf.setFont('helvetica', 'italic');
    pdf.setFontSize(9);
    pdf.setTextColor(107, 114, 128);
    pdf.text('This analysis is based on metadata comparison and should be used in conjunction with visual inspection.', 
             margin + 5, y + 35);
